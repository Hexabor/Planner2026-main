        ...
        const CHANGELOG = [
            {
                ...
                items: [
                    '🔧 Planner → panel "NOTAS DEL DÍA": ya no se solapa con la tarjeta de controles. Ahora es un panel independiente que solo aparece cuando hay alguna nota ese día. Además, las notas se pueden escribir/editar aunque el día esté cerrado (antes lo impedía el mismo aviso que bloquea cambiar turnos)',
                    '📝 Planner → nuevo botón "Nota" junto a "Blindar" bajo la paleta de turnos: actívalo y haz clic en un turno (rejilla o balance) para escribir una anotación libre (p.ej. por qué está blindado, o por qué se asignó así). El turno muestra un 📝 con la nota al pasar el ratón, y todas las notas del día se ven en el panel "NOTAS DEL DÍA", donde también se pueden editar o borrar',
                    '🔒 Planner → nuevo botón "Blindar" bajo la paleta de turnos (ahora compacto, con ⓘ explicativo): actívalo y haz clic en un turno (rejilla o balance) para blindarlo. Un turno blindado muestra un candado y, si alguien intenta pintarlo, borrarlo, arrastrarlo o intercambiarlo, pide confirmación antes de quitar el blindaje — reversible en cualquier momento con el mismo clic. Las acciones masivas (vaciar día/semana, rellenar, pegar patrón, limpiar turnos fuera de contrato) respetan los turnos blindados y los saltan',
            ...
            },
            {
                ...
                items: [
                    ...
                    '✨ Columna "Fest." = festivos disfrutados (la semana tuvo 2 libranzas sin contar el festivo; los "coincide" no cuentan)',
                    '✨ Columna "Ant." = festivos del tramo anterior no compensados dentro de ese tramo (su R cae en el rango actual o no existe), con tooltip de fechas y motivo',
                    '✨ Columna "F+R-A" = festivos disfrutados + recuperaciones − Ant (resta la deuda heredada del tramo anterior; en rojo si es negativa)',
