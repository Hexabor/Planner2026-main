...
Object.assign(App.ui, {
        renderShifts: function(c) {
            let html = `<div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px;">
                <h2 style="margin:0; font-size:1.2rem;">Catálogo de Turnos</h2>
                <div style="display:flex; gap:10px;">
                    <button class="btn btn-header" onclick="App.ui.showCustomGallery()">📊 Analizar personalizados</button>
                    <button class="btn btn-primary" onclick="App.logic.shiftSelect(null)">+ Nuevo Turno</button>
                </div>
            </div>`;
            if(App.data.shiftDefs.length === 0) { html += `<div style="text-align:center; padding:40px; border:2px dashed #e2e8f0; border-radius:8px; color:#94a3b8">No hay turnos.</div>`; } else {
        ...
        showCustomGallery: function() {
            ...
            const container = document.getElementById('custom-gallery-content');
            
            if(stats.length === 0) {
                container.innerHTML = `
                    <div style="text-align:center; padding:60px 20px; background:#f8fafc; border:2px dashed #e2e8f0; border-radius:8px;">
                        <div style="font-size:3rem; margin-bottom:15px;">✨</div>
                        <h3 style="color:var(--text-muted); margin:0 0 10px 0;">No hay turnos personalizados</h3>
                ...
                let html = `
                    <div style="margin-bottom:15px; padding:12px; background:#f0f9ff; border:1px solid #bae6fd; border-radius:6px; display:flex; justify-content:space-between; align-items:center;">
                        <div style="font-size:0.85rem; color:#0369a1;">
                            <strong>ℹ️ Información:</strong> Estos son turnos personalizados que no coinciden exactamente con ningún turno del catálogo.
                        </div>
                        <div style="font-size:0.8rem; color:#475569; font-weight:600;">
                            Mostrando ${startIdx + 1}-${endIdx} de ${stats.length} turnos
                        </div>
                    </div>`;
                
                // Controles de paginación (arriba)
                if(totalPages > 1) {
                    html += `<div style="display:flex; justify-content:center; gap:8px; margin-bottom:15px; align-items:center;">`;
                    
                    // Botón anterior
                    html += `<button onclick="App.ui.changeCustomPage(-1)" 
                                    ${currentPage === 0 ? 'disabled' : ''} 
                                    style="padding:6px 12px; border:1px solid #e2e8f0; background:white; border-radius:4px; cursor:${currentPage === 0 ? 'not-allowed' : 'pointer'}; opacity:${currentPage === 0 ? '0.5' : '1'};">
                                ◀
                            </button>`;
                    ...
                    html += `<button onclick="App.ui.changeCustomPage(1)" 
                                    ...
                                    style="padding:6px 12px; border:1px solid #e2e8f0; background:white; border-radius:4px; cursor:${currentPage === totalPages - 1 ? 'not-allowed' : 'pointer'}; opacity:${currentPage === totalPages - 1 ? '0.5' : '1'};">
                                ▶
                            </button>`;
                ...
                pageStats.forEach((custom, idx) => {
                    const realIdx = startIdx + idx; // Índice real en el array completo
                    ...
                    // Vista previa usando renderPlannerTimeline
                    const mockShift = {
                        start: custom.start,
                        end: custom.end,
                ...
                // Controles de paginación (abajo) - solo si hay varias páginas
                if(totalPages > 1) {
                    html += `<div style="display:flex; justify-content:center; gap:8px; margin-top:15px; align-items:center;">`;
                ...
                html += `
                    <div style="margin-top:20px; padding:15px; background:#fef3c7; border:1px solid #fde047; border-radius:6px;">
                        <div style="font-size:0.8rem; color:#92400e;">
                            <strong>💡 Consejo:</strong> Si un turno personalizado se repite mucho, considera agregarlo al catálogo para facilitar su gestión.
                        </div>
                    </div>`;
            ...
            document.getElementById('custom-gallery-modal').classList.add('open');
        },
