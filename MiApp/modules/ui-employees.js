            ...
            if(App.data.empleados.length===0) html+=`<div style="text-align:center;padding:40px;color:#94a3b8">Sin empleados.</div>`;
                ...
                list.forEach((e, idx) => {
                    ...
                    if(!isPrefs) {
                        ...
                        const rolHoy = (typeof Utils.getRolEnFecha === 'function') ? Utils.getRolEnFecha(e, new Date().toISOString().slice(0,10)) : e.rol;
                        ...
                        html+=`<td><span class="badge badge-role">${rolHoy}</span></td><td><span class="badge badge-tag">T${tag}</span></td><td><div class="contract-chart" style="background:${bg}"></div>${contratoActual}h${tieneHistorial?'<sup style="font-size:0.55rem;color:#94a3b8;margin-left:2px;">📋</sup>':''}</td>`;
        ...
        content = `
            ...
            <div style="display:grid; grid-template-columns: 1fr 100px; gap:12px; margin-bottom:20px;">
                <div>
                    ...
                    <div style="background:#eff6ff; color:#2563eb; font-family:monospace; font-weight:800; font-size:14px; height:42px; display:flex; align-items:center; justify-content:center; border-radius:6px; border:1px solid #dbeafe;" title="El rol viene determinado por el tramo de contrato activo">${Utils.getRolEnFecha(e, hoy)}</div>
            ...
            <div style="background:#fff; border:1px solid #2563eb; border-radius:10px; padding:12px; margin-bottom:20px;">
                ...
                <div id="contratos-container">
                    ${e.contratos.map((t, i) => {
                        ...
                        const rolOpts = [['MNG','Manager'],['AM','Asst. Mgr'],['SPV','Supervisor'],['STF','Staff']]
                            .map(([v,l]) => `<option value="${v}" ${tRol===v?'selected':''}>${l}</option>`).join('');
                        const dpDesde = Utils.getDatePickerHTML(t.desde, `App.logic.updateContrato('${id}', ${i}, 'desde', this.value); App.ui.markDirty()`);
                        const dpHasta = Utils.getDatePickerHTML(t.hasta || '', `App.logic.updateContrato('${id}', ${i}, 'hasta', this.value); App.ui.markDirty()`);
                        return `
                        <div style="background:#f8fafc; padding:8px 10px; border-radius:6px; border:1px solid #e2e8f0; margin-bottom:8px;">
                            <div style="display:grid; grid-template-columns:1fr 1fr 22px; gap:6px; align-items:center; margin-bottom:6px;">
                                <div>
                                    <div style="font-size:9px;color:#94a3b8;font-weight:700;text-transform:uppercase;margin-bottom:3px;">Desde</div>
                                    ${dpDesde}
                                </div>
                                <div>
                                    <div style="font-size:9px;color:#94a3b8;font-weight:700;text-transform:uppercase;margin-bottom:3px;">Hasta</div>
                                    ${dpHasta}
                                </div>
                                <button type="button" onclick="App.logic.removeContrato('${id}', ${i}); App.ui.renderEmpInspector('${id}'); App.ui.markDirty();" style="border:none;background:none;color:#ef4444;font-size:18px;cursor:pointer;padding:0;line-height:1;align-self:flex-end;">&times;</button>
                            </div>
                            <div style="display:grid; grid-template-columns:1fr 90px; gap:6px; align-items:center;">
                                <div>
                                    <div style="font-size:9px;color:#94a3b8;font-weight:700;text-transform:uppercase;margin-bottom:3px;">Rol</div>
                                    <select onchange="App.logic.updateContrato('${id}', ${i}, 'rol', this.value); document.getElementById('ie-rol').value=this.value; App.ui.markDirty();" style="width:100%; padding:5px 6px; border:1px solid #cbd5e1; border-radius:4px; font-size:12px; font-weight:700;">${rolOpts}</select>
                                </div>
                                <div>
                                    <div style="font-size:9px;color:#94a3b8;font-weight:700;text-transform:uppercase;margin-bottom:3px;">Horas</div>
                                    <input type="number" step="0.01" value="${toDec(t.horas)}" onchange="App.logic.updateContrato('${id}', ${i}, 'horas', this.value); App.ui.markDirty();" style="width:100%; text-align:right; font-weight:800; font-size:13px; color:#2563eb; border:1px solid #2563eb; border-radius:4px; padding:5px 6px;">
                                </div>
                            </div>
                        </div>`;
                    }).join('')}
